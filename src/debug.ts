 ...
 * 2. URL 参数   ：?debug=editor / ?debug=1 / ?debug=0
 * 3. 环境变量   ：VITE_DEBUG / DEBUG / NODE_DEBUG（Node 与 SSR 场景）
 *
 * 并补齐包本身不支持的「通配符 / 短名」匹配，例如 `note-editor:*`、`editor`。
...
export const DEBUG_NAMESPACES = ['note-editor:editor', 'note-editor:demo'] as const;
 ...
 * 前缀（'note-editor:*'）、短名（'editor' 命中 'note-editor:editor'）、通配（'*editor*'）。
 */
...
/**
 * 逗号分隔的批量开关，语义同 debug 包：`'editor,-note-editor:demo'`。
...
export function describeDebug(): string {
  ...
  return `note-editor debug: ${on.length ? on.join(', ') : '（全部关闭）'}`;
}
...
export const logEditor = createLogger('note-editor:editor');
export const logDemo = createLogger('note-editor:demo');
